                ...
                for line in block["lines"]:
                    for span in line["spans"]:
                        ...
                        if original_text.strip():  # Traducir solo si hay texto
                            ...
                            # Obtener las coordenadas del texto original
                            x0, y0, x1, y1 = span["bbox"]

                            # Ajuste del texto dentro de los márgenes de la página
                            page_width = page.rect.width
                            page_height = page.rect.height
                            if x1 > page_width:  # Si el texto se sale de los límites horizontales
                                x1 = page_width - 10  # Ajustar la posición horizontal
                            if y1 > page_height:  # Si el texto se sale de los límites verticales
                                y1 = page_height - 10  # Ajustar la posición vertical

                            # Ajustar el tamaño de la fuente si el texto es largo
                            font_size = span["size"]
                            if len(translated_text) > len(original_text):
                                font_size *= 0.8  # Reducir tamaño si el texto traducido es largo

                            # Eliminar el texto original (cubrir con un rectángulo blanco)
                            page.draw_rect(span["bbox"], color=(1, 1, 1), fill=(1, 1, 1))  # Rellenar con blanco
                            ...
                            page.insert_text(
                                (x0, y0),  # Coordenadas originales ajustadas
                                translated_text,
                                fontsize=font_size,  # Tamaño ajustado de fuente
